...
if __name__ == '__main__':
	...
	for file, predicted_files in zip(data_files, result_files):
		...
		,'drawee_bank_country','doc_settlement_instructions','document_enclosed', "stamp", "signature"]
	...
	df.loc[((df["label_name"].isin(['beneficiary_address','payment_terms_terms_of_delivery_&_payment',
    'declaration','vessel_flight_no',"remitter_address","consignee_address", "description_of_goods", "vessel_name", "notify_party_address"])) & (df["Accuracy"].apply(int) >=60)), "Match/No_Match"] = 1
	...
	if not os.path.exists(res_path):
		os.mkdir(res_path)
	df.to_csv(f'{res_path}/BOL_analysis_pre_valid_May30_after_fuzzy_match_post-processing.csv')
