...
idp_inv_images_folder = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/Raw_ouput_data/FINAL_merged_classified_images/Bill_of_Lading"
idp_inv_labels_folder = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/annoations/labels_BOL"
idp_inv_ocr_folder = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/ocr_data"
classes_path = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/annoations/classes_BOL.txt"
annot_classses_file= "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/annoations/classes_BOL.txt"
idp_inv_json_results = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/Raw_ouput_data/bb_scripts/BOL_output"
idp_inv_image_results = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/Benchmarking_Results/BOL_eval_results"
csv_file_path = "/datadrive2/IDF_AL_MASRAF/ITF_Al_Masraf_Classification/Data/Benchmarking_Results/BOL_eval_results"
plot_gt_flag = True
...
def read_ocr_data(ocr_file):
    ocr_file_path = os.path.join(idp_inv_ocr_folder, ocr_file)
    print("OCR File : ", ocr_file_path)
    
    # Check if the file actually exists before trying to open it
    if not os.path.exists(ocr_file_path):
        print(f"WARNING: Missing OCR file for {ocr_file}. Skipping this image.")
        return None
    
    with open(ocr_file_path, 'r') as f:
        ocr_dict = eval(f.read())
        
    return ocr_dict['ocrContent']
...
#new function added to handle the idp json strcuture and ocr 
def parse_prediction_json(idp_results):
    formatted_preds = []
    
    # Safely navigate the new JSON structure
    input_file = idp_results.get("response", {}).get("input_file", {})
    
    keys_extraction = input_file.get("keys_extraction", {})
    keys_bboxes = input_file.get("keys_bboxes", {})
    keys_confidence = input_file.get("keys_confidence", {})

    # Loop through the bounding boxes dictionary
    for class_name, bboxes in keys_bboxes.items():
        # Get the matching text and confidence lists
        texts = keys_extraction.get(class_name, [])
        confs = keys_confidence.get(class_name, [])

        for idx, coord in enumerate(bboxes):
            # Match the box with its text and confidence by index
            val = texts[idx] if idx < len(texts) else ""
            conf = confs[idx] if idx < len(confs) else 0.0
            
            # Append in the exact format your script expects: [bbox, class, confidence, text]
            formatted_preds.append([coord, class_name, conf, val])
...
def detect(save_csv=False):
    ...
    with open(csv_path, mode='w', newline='', encoding='utf-8') as file:
        ...
        for file in os.listdir(idp_inv_images_folder):
            ...
            # 1. Get formatted predictions from the new JSON
            pred = parse_prediction_json(idp_results)
            
            # 2. Update the prediction counter
            total_pred_labels += len(pred)

            # 3. Read the Ground Truth Labels
            print("Label File : ", f"{file.rsplit('.', 1)[0]}.txt")
            label_data = read_text_label_data(f"{file.rsplit('.', 1)[0]}.txt")

            # 4. Read the External OCR Data
            ocr_filename = f"{file.rsplit('.', 1)[0]}_textAndCoordinates.txt"
            ocr_data = read_ocr_data(ocr_filename)
        ...
        # Force the Excel reports to save in your main output folder instead of "geo_reports"
        save_overall_geo_report(total_actual_labels, total_pred_labels, fuzz25_correct_preds, fuzz50_correct_preds, 
                        fuzz75_correct_preds, fuzz85_correct_preds, fuzz90_correct_preds, fuzz100_correct_preds, folder_name=out)
        save_fuzzy_results(label_wise_total_pred_count, label_wise_fuzz75_pred_count, folder_name=out, filename="fuzzy75_results.xlsx")
        save_fuzzy_results(label_wise_total_pred_count, label_wise_fuzz100_pred_count, folder_name=out, filename="fuzzy100_results.xlsx")
        save_fuzzy_results(label_wise_total_pred_count, label_wise_fuzz90_pred_count, folder_name=out, filename="fuzzy90_results.xlsx")
