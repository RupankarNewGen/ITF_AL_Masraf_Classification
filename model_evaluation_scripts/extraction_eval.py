...
idp_inv_images_folder = "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/images"
idp_inv_labels_folder = "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/lables"
#idp_inv_ocr_folder = "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/ocr"
classes_path = "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/classesPL.txt"
annot_classses_file= "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/classesPL.txt"
idp_inv_json_results = "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/result"
idp_inv_image_results = "/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/result"
csv_file_path = '/home/lpt6964/Downloads/Geo_LayoutLM-geo_code_latest_timeoptm_version_updated_apr22_sharing_backend/inference_main/code/test_data/result'
plot_gt_flag = False
...
#     # Extract the list from the 'ocrContent' key
#     ocr_data = ocr_dict['ocrContent']
#     return ocr_data
...
def parse_prediction_json(idp_results):
    """
    Extracts predictions and OCR data directly from the new JSON format.
    """
    # Grab the first key's value (the filename's dictionary)
    file_data = list(idp_results.values())[0]
    
    extraction_result = file_data.get("extraction_result", {})
    
    formatted_preds = []
    
    # Loop through the new extraction_result dictionary
    for class_name, data in extraction_result.items():
        # Skip empty dictionaries {} or keys that have no coordinates []
        if not data or not data.get("coordinate"):
            continue
            
        coords = data.get("coordinate", [])
        confs = data.get("model_confidence", [])
        val = data.get("value", "")
        
        # Some fields might have multiple bounding boxes
        for idx, coord in enumerate(coords):
            # Safely grab the confidence score if it exists, otherwise default to 0
    ...
    # Extract the OCR data directly from the new JSON
    ocr_data = file_data.get("ocrInfo", {}).get("Text_and_Coordinates", [])
    
    return formatted_preds, ocr_data
...
def detect(save_csv=False):
    ...
    with open(csv_path, mode='w', newline='', encoding='utf-8') as file:
        ...
        for file in os.listdir(idp_inv_images_folder):
            ...
            # raw_pred = parse_prediction_json(idp_results)
            ...
            # 1. Get formatted predictions AND OCR data directly from the new JSON
            pred, ocr_data = parse_prediction_json(idp_results)
