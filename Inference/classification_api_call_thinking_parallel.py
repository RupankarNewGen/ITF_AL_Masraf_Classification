...
def call_single_image(img_path, output_folder, target_dpi, target_size):
    """Processes a single image; skips if non-error JSON already exists."""
    img_start = time.perf_counter()
    filename = os.path.basename(img_path)
    name_only = os.path.splitext(filename)[0]
    save_path = os.path.join(output_folder, f"{name_only}.json")

    # LAYER 1: Resume Logic (Checks the output folder)
    if os.path.exists(save_path):
        try:
            with open(save_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                # If it's NOT an error, we skip. If it IS an error, we re-process.
                if data.get("classification") != "error":
                    return f"Skipped (Success Exists): {name_only}"
        except: 
            pass # If file is corrupted, we re-process

    client = OpenAI(base_url=VLLM_API_URL, api_key="sk-vllm")
    try:
        img_64 = encode_img_b64(img_path, target_dpi=target_dpi, target_size=target_size)
    ...
    return f"Processed: {name_only} in {duration:.2f}s"

def process_parallel(manifest_path, output_folder, target_dpi, target_size, workers, exclude_files):
    os.makedirs(output_folder, exist_ok=True)
    
    with open(manifest_path, 'r', encoding='utf-8') as f:
        image_paths = json.load(f)

    # LAYER 2: Global Exclusion Logic (Pre-filtering)
    exclude_set = set()
    if exclude_files:
    ...
    print(f"Total images to evaluate: {len(filtered_paths)} with {workers} workers.")
    
    total_start_time = time.perf_counter()
    completed_count = 0

    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(call_single_image, path, output_folder, target_dpi, target_size): path for path in filtered_paths}
        
        for future in tqdm(as_completed(futures), total=len(filtered_paths), desc="Overall Progress"):
            try:
                ...
                if completed_count % 100 == 0:
                    ...
                    print(f"\n" + "="*40)
                    print(f"MILESTONE: {completed_count} images processed.")
                    print(f"Time elapsed: {current_lap_time:.2f}s")
                    print("="*40 + "\n")
