...
def main():
    ...
    # return_dict= dataset[0]
    # print(f'dataset: {return_dict}')
    # exit('+++++++++++++')
    # print(dir(dataset))
    # print(vars(dataset))
    # exit('+++++++++++++++++++')
    ...
    for example_idx, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        # print(f'head output: {head_outputs}')
        # print(f'loss dict: {loss_dict}')
        # print(loss_dict)
        # exit('+++++++++++++++')
        step_out = do_eval_step(batch, head_outputs, loss_dict, eval_kwargs, dump_dir=cfg[mode].dump_dir)
        print(step_out["labeling"])
        # print(len(step_out['labeling']['gt_str_list'][0]))
        # print(len(step_out['labeling']['pr_str_list'][0]))
        # exit('+++++++++++++++++=')
        step_outputs.append(step_out)
    # print(f'step outs: {step_outputs}')
    # exit('++++++++++++++++++==')
