		...
		if length > 1:
			...
			while i in range(length - 1):
				...
				if (min_dist_horizontal <= eps_horizontal or IOU_horizontal > 0 or inter_percentage > 0 ) or \
					...
					all_values.insert(i, new_value)
					need_to_remove=[]
					# updated_all_values= all_values
					print(f'new value has added: {all_values}')
					for j in range(1,len(all_values)-1):
						print(j)
						print(all_values[0][1])
						print(all_values[j][1])
						bb1_area= calculate_bounding_box_area(all_values[0][1])
						bb2_area= calculate_bounding_box_area(all_values[j][1])
						inter_per = get_intersection_percentage(all_values[0][1], all_values[j][1])
						print(f'bounding box bb1 area: {bb1_area}')
						print(f'bounding box bb2 area: {bb2_area}')
						print(f'intersection area: {inter_per}')
						
						if bb1_area>bb2_area and inter_per==1:
							need_to_remove.append(all_values[j])
							# all_values.remove(all_values[j])
						j= j+1
					print(need_to_remove)
					# updated_all_values=[]
					# updated_all_values=[updated_all_values.append(item) for item in all_values if item not in need_to_remove]
					# all_values= updated_all_values
					for item in need_to_remove:
						all_values.remove(item)
					print(f'after removing: {all_values}')
					length = len(all_values)
				...
				# 			bb1_area= calculate_bounding_box_area(all_values[i][1])
				# 			bb2_area= calculate_bounding_box_area(all_values[i+1][1])
