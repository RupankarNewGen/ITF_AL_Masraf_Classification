...
class masterDataPrep():
	def __init__(self, folder_path:str, image_folder:str='Images', master_labels_folder:str='Master_Labels', 
	merged_master_folder:str='New_Master_Data_Merged') -> None:
	
		""" There are multiple bounding boxes for the same label form example: address field as a part
		of ground truth preparation need to merge into a single filed
		
		Params:
		folder_path: Root folder of the dataset
		image_folder: Name of the Image folder name 
		master_labes_folder: Name of the master label folder
		merged_master_folder: results path folder name	
		"""
		self.folder_path= folder_path
		self.image_folder= image_folder
		self.master_labels= master_labels_folder
		self.merged_master_folder= merged_master_folder
		self.images_path= os.path.join(self.folder_path, self.image_folder)
		self.master_data_path= os.path.join(self.folder_path, self.master_labels)
		self.data_path = os.path.join(self.folder_path, self.merged_master_folder)
		self.image_files = self.__loadData__()
		for file in self.image_files:
			with open(os.path.join(self.master_data_path, file+"_labels.txt"), "r") as f:
				labels = json.load(f)
			self.merge_surrounding(labels)
			with open(os.path.join(self.data_path, file+'_labels.txt'), "w") as f:
				json.dump(labels, f)
	...
	def minimum_distance(self, bb1, bb2):
		# bb1 points
		assert bb1[0] < bb1[2]
		assert bb1[1] < bb1[3]
		assert bb2[0] < bb2[2]
	...
	def model_output_sum(self,key, box, data):
		all_values = data[key]
		text_result = ""
		for value in all_values:
			# print(type(value[1]))
			try:
				inter_percent = self.get_intersection_percentage(box, value[1])
	...
	def minimum_distance_horizontal(self, bb1, bb2):
	...
	def minimum_distance_vertical(self, bb1, bb2):
	...
	def get_iou_horizontal(self, bb1, bb2):
	...
	def get_iou_vertical(self, bb1, bb2):
	...
	def calculate_bounding_box_area(self, box_coords):
		"""
	...
	def merge_surrounding(self, data):
		for key in list(data.keys()):
			...
			print(data[key])
			print(len(data[key]))
			sorted_tokens = self.vertical_sort(data[key])
			...
			if length > 1:
				...
				while i in range(length - 1):
					...
					min_dist_horizontal = self.minimum_distance(bb1, bb2)
					min_dist_vertical = self.minimum_distance_vertical(bb1, bb2)
					...
					# exit('++++++++++++')
					try:
						IOU_horizontal = self.get_iou_horizontal(bb1, bb2)
						IOU_vertical = self.get_iou_vertical(bb1, bb2)
						inter_percentage = self.get_intersection_percentage(bb1, bb2)
						# print(key)
						# print(f'minimum distance vertical : {min_dist_vertical}')
						# print(f'minimum distance horizental : {min_dist_horizontal}')
						# print(f'bounding box 1: {bb1} and bounding box 2: {bb2}')
					...
					if inter_percentage!=1.0:
						if (min_dist_horizontal <= eps_horizontal or IOU_horizontal > 0 or inter_percentage > 0) or \
							...
							text = self.model_output_sum(key, box, data)
							new_value = [text, box]
	...
	def __loadData__(self):
		# Main function
		data_path = os.path.join(self.folder_path, "New_Master_Data_Merged")
		...
		self.image_files = [x.split("_linking.png")[0] for x in self.image_files]
...
# 	obj= masterDataPrep(folder_path, 'Images','Master_Labels','New_Master_Data_Merged')
# 	obj.__loadData__()
