...
def natural_sort_key(s):
    ...
    It splits 'page_10' into ['page_', 10] so it sorts correctly.
    """
...
def collect_dynamic_siblings(input_json_path, output_json_path, target_filenames, n_avg=25):
    ...
    - Uses Natural Sort to ensure page sequences are correct.
    - Outputs a nested JSON { folder_name: [file_paths] }.
    """
    ...
    for folder in target_parent_folders:
        ...
        # Fetch files and apply NATURAL SORT
        files = [os.path.abspath(os.path.join(folder, f)) for f in os.listdir(folder) 
                 if os.path.splitext(f)[1].lower() in valid_extensions]
        
        # This fix ensures Page 2 comes after Page 1, not after Page 19
    ...
    # 5. Organise paths as per folder (Nested Dictionary)
    ...
    for folder, count in current_allocations.items():
        ...
        print(f"{folder_name:<30} | {len(folder_inventory[folder]):<10} | {count:<10}")
        
        # Get the first 'n' pages in sequence
        final_output_structure[folder_name] = folder_inventory[folder][:count]
        total_count += count

    # 6. Save nested results
    with open(output_json_path, 'w', encoding='utf-8') as f:
        json.dump(final_output_structure, f, indent=4)

    print("-" * 50)
    print(f"TOTAL IMAGES FETCHED: {total_count}")
    print(f"NESTED MANIFEST SAVED TO: {output_json_path}")
...
if __name__ == "__main__":
    ...
    OUT_JSON = "targeted_sibling_expansion_nested.json"
